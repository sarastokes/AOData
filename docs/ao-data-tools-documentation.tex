...
initializeAOData();
        \end{lstlisting}
        You only need to initialize AOData once. Rerun this if you move the location of the AOData folder on your computer. This function stores information about AOData's location on your computer in MATLAB's user preferences. After completion it opens \aodfcn{PackageManagerApp}. If you are using any user-defined AOData packages, add them. This is an important step because AOData If you create a new package after initialization, you can return 
        \\$\quad$\\ 
...
clear import
	...
	\subsection{Object Oriented Programming}
		...
		If you aren't familiar with OOP, this will be the biggest learning curve for using AOData. The tutorials have information on how to use object-oriented programming within the context of AOData and there are also resources listed in \ref{section:CodeDoc}.  
		
		Once you know what the code is supposed to do, classes can be way easier than functions and scripts.  Below I describe some of the key concepts of OOP, because they are also key benefits of taking the time to learn more about OOP.
		\begin{enumerate}
			\item \textbf{Polymorphism:} For example when you can concatenate \aodclass{char} and \aodclass{double} in MATLAB, you use the same function, \aodfcn{cat()}. This is because both classes define a \aodfcn{cat()} method. Without classes, this would require two separate functions with different names, \aodfcn{concatenateDouble()} and \aodfcn{concatenateChar()}. So classes standardize behavior and make code way easier to write and remember. % consistent interface
			\item \textbf{Inheritance:}
			\item \textbf{Encapsulation:}
			\item \textbf{Abstraction:} OOP allows you to hide complicated code from end-users and build a simple, easy interface for interacting with that code base. Keep in mind, you are your own ``end-user'' -- you may understand everything about a piece of code when you first write it, but in a few months or years when those details are no longer fresh in your memory, you will be the chief beneficiary of the simple, easy interface.
		\end{enumerate}
	...
	\subsection{Entity locations within the HDF5 file}
		...
		\begin{tikzpicture}[scale=0.95]
			...
			\FTOdir(\FTroot,root,Experiment){       
				...
				\FTdir(root,Segmentations,Segmentations - container for all Segmentations within the Experiment){}  
