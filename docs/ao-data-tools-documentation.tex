...
\section{Introduction}
	...
	\subsection{Problems}	
		...
		\begin{enumerate}
			...
			\item Our code changes over time and we need to be free to make necessary changes without fear of breaking past code. % git and aod.common.Attributes
...
obj = MyClass(inputs)
	...
	\begin{itemize}
		\item \aodprop{MyProp}
	\end{itemize}
	\docheader{Parameters:} Optional inputs sent to \aodclass{aod.common.Attributes} and written as attributes
	\begin{itemize}
...
obj = aod.core.Entity(name)
		...
		\begin{itemize}
			...
			\item \aodprop{parameters} - \aodclass{aod.common.Attributes} containing entity metadata
			\item \aodprop{files} - \aodclass{aod.common.Attributes} containing file names associated with the entity
...
classdef MyFactory < aod.util.Factory
	...
	\subsection{aod.common.Attributes \myparent{containers.Map}}
		\noindent  This is a wrapper for MATLAB's builtin \aodclass{containers.Map} class, so you if know how to use that then you know how to use \aodclass{aod.common.Attributes}. The only differences are better command line display (each key and the associated values are displayed) and a standardized approach for writing to HDF5 files. 
...
\section{HDF5 File Details}
	...
	\noindent High-level summary: When you write an entity to an HDF5 file, the entity will be written as a \textbf{group}. Public properties will be written as  \textbf{datasets} within the group. Anything in a \aodclass{aod.common.Attributes} object will be written as an \textbf{attribute} of the group. Properties referring to other entities (e.g. Parent) are written as \textbf{references}.  Several entities also contain default groups which are \textit{containers} for other groups (e.g. Experiments have Calibrations, a container group for all the individual Calibration groups). 
	...
	\subsection{Reading and Writing Entities to HDF5 Files}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item Several entities have a \aodprop{files} property of class \aodclass{aod.common.Attributes}. The contents are written as attributes of a text dataset with some placeholder text.
			...
			\item The contents of \aodclass{aod.common.Attributes} classes are written as attributes of the entity group.
			\begin{itemize}
