...
\newcommand{\ctext}[1]{\texttt{\textcolor{green4}{#1}}}
...
\section*{1.1. Entity (\texttt{aod.core.Entity})}
    ...
    \begin{PropFlag}[Dependent, GetAccess=public]
        ...
        \propd{l}{Automated group name used if \texttt{Name} is not set. See \ref{fcn:cEntityGetLabel}}\\
        &\\
        ...
        \propd{Dataset specifications (see \ret{specifyDatasets})}\\
    \end{PropFlag}
...
%* SETNAME
    ...
    \begin{example}
        ...
        \begin{CodeBox}
            ...
            $\quad$\\
            \ccomm{\% Remove the name and group name will now be the output of \ref{fcn:cEntityGetLabel}.}\\
            obj.\textbf{setName}([]);\\
            >> disp(obj.groupName);\\
            \hspace{3ex}`Analysis'
        \end{CodeBox}
...
%* GETLABEL
    \begin{ProtectTab}[getLabel()]
        \customlabel{fcn:cEntityGetLabel}{\mymethod{getLabel()}}
    ...
    \begin{example}
        Naming each pinhole individually would be time-consuming and could lead to heterogeneous naming conventions. The \ref{fcn:cEntityGetLabel} method is used by \ret{Pinhole} to define the \propref{label} property based on the pinhole diameter. 

        $\quad$\\
        \noindent\texttt{\raggedright 
        \defblock{properties} \textit{(Access = protected)}\\
        \ONE\defblock{function} value = \textbf{getLabel}(obj)\\
        \ONE\ONE value = [\ctext{`Pinhole\_'}, \mclass{num2str}(obj.diameter), \ctext{`um'}];\\ 
        \ONE\defblock{end}\\
        \defblock{end}}
    \end{example}\myspace
...
%* GETFROMEPOCH
    \begin{MethodTab}[getFromEpoch()]
        \customlabel{fcn:cExperimentGetFromEpoch}{\mymethod{getFromEpoch()}}
        Search the child entities of a specific epoch(s).
    \end{MethodTab}\vspace{1ex}

    \begin{CtorBox}
        \textbf{getFromEpoch}(obj, epochIDs, entityType, \textit{varargin})
    \end{CtorBox}\vspace{1ex}
...
%! AOD.CORE.SOURCE
%! ---------------
\section*{1.4. Source (\texttt{aod.core.Source})}\customlabel{cls:cSource}{\myclass{aod.core.Source}}
    \noindent {\large Represents the source of acquired data (e.g.) subject, eye, location etc.}\vspace{1ex}
    ...
    \begin{CtorTab}[Source()]
        The source of the imaging data (e.g., eye, location, subject, etc.)
    \end{CtorTab}\vspace{1ex}
...
\section*{2.1. Entity (\texttt{aod.persistent.Entity})}\customlabel{cls:pEntity}{\myclass{aod.persistent.Entity}}
    ...
    \begin{PropFlag}[Hidden, SetAccess=private]
        ...
        %\propd{The generated label for the entity. See \ref{fcn:cEntityGetLabel}}\\
        %&\\
